...
from audio.audio import Audio as Audio_Abstract
...
def average_spectrum(audio_object, **kwargs):
    ...
    convert_to_dB = kwargs.get('convert_to_dB', False)
    if convert_to_dB:
...
def power_spectral_density(audio_object, freq_range=(0, 24000), nperseg=4096, display=False, norm=False):

    # Select single channel if multi-channel audio (e.g., take first channel)
    data = audio_object.data[0] if audio_object.num_channels > 1 else audio_object.data

    # Calculate PSD
    freqs, psd = welch(data, fs=audio_object.sample_rate, nperseg=nperseg)
